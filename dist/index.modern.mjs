...
const updateNestedObjectDefaultValue = (obj, keyToFind, newValue) => {
...
async function resolveWebformContent(id, locale, drupal, fetchOptions) {
  const url = drupal.buildUrl(`/webform/${id}?_format=json`);
  // Suport for Multilanguage context.
  const elementsUrl = drupal.buildUrl(`${locale !== '' ? `/${locale}` : ''}/webform_rest/${id}/elements?_format=json`);
...
async function resolveWebformSubmission(id, sid, locale, drupal, fetchOptions) {
  const url = drupal.buildUrl(`/webform/${id}?_format=json`);
  // Suport for Multilanguage context.
  const elementsUrl = drupal.buildUrl(`${locale !== '' ? `/${locale}` : ''}/webform_rest/${id}/elements?_format=json`);
  const submissionUrl = drupal.buildUrl(`/webform_rest/${id}/submission/${sid}?_format=json`);
  const [result, elementsResult, submissionResult] = await Promise.all([drupal.fetch(url.toString(), _extends({}, fetchOptions, {
    headers: {
      'Content-Type': 'application/json'
    }
  })), drupal.fetch(elementsUrl.toString(), _extends({}, fetchOptions, {
...
const _excluded = ["data", "id", "sid", "customComponents", "onSubmit", "apiUrl", "validate"];
